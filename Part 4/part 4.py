...
def part():
    file=open('kosindu.txt','w') #creating the text file
    for i in mylist:
        for k in i:
            file.write(k+'\n')
    file.close()
    file = open('kosindu.txt','r')
    print(file.read())
    print()
...
def student():
    ...
    if total != 120:
        ...
        print("Progress")
        pro_count+=1
        alist=[f"progress-{Pass},{defer},{fail}"]
        mylist.append(alist)
    elif fail>=80:
        print("Exclude")
        exclude_count+=1
        alist=[f"Exclude-{Pass},{defer},{fail}"]
        mylist.append(alist)
    elif Pass==100:
        print("Progress(module trailer)")
        trail_count +=1
        alist=[f"Progress-module trailer-{Pass},{defer},{fail}"]
        mylist.append(alist)
    else:
        print("Do not progress - modile retriever")
        retri_count+=1
        alist=[f"Do not progress - modile retriever-{Pass},{defer},{fail}"]
        mylist.append(alist)
...
def staff():
    while True:
        ...
        if option=='y':
            ...
            part() #calling the text file function
            break
        else:
